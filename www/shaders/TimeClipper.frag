...
void main()
{
	float index		= mod(floor(deltaTime * framesPerSecond / 1000.0), cols * rows);	// Figure out which frame to display.
	
	vec2 location 	= vec2(mod(index, cols), mod(floor(index/cols),rows));
